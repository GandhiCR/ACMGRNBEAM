...
Public Class beamwisemultiple
    Inherits System.Web.UI.Page
    Dim grndate, dcdate, autonumdate As DateTime
    Dim docnumber, rowcount As Integer
    Dim seqnumber, autonumber, selectedautonumber, selectedproduct As String
    Dim grnquantity, grnmeters, grnlength, totalquantity, totalmeters As Decimal
    Dim grndocentry, username As String
    ...
    Public Sub getgrndetails()
        ...
        End If
        Dim sql1 As String = "select B.U_ItemCode as [Item Code],B.U_ItemName as [Item Name],A.CardCode as [Ven. Code],A.CardName as [Vendor Name] from OPDN A INNER JOIN PDN1 B On A.DocEntry = B.DocEntry where A.DocNum = '" & txtgrn.Text & "' and A.DOCENTRY = '" & grndocentry & "' and b.u_itemcode is not null"
        ...
        con.Close()
    End Sub
    Protected Sub radio_Changed(sender As Object, e As EventArgs)
        For Each row As GridViewRow In GridView1.Rows
            If row.RowType = DataControlRowType.DataRow Then
                ...
                If chkRow.Checked = True Then
                    selectedproduct = row.Cells(2).Text
                    loadgrndetails()
                End If
            End If
        Next
    ...
    Public Sub loadgrndetails()
        grnpanel.Visible = True
        Dim sql1 As String = "select A.DocNum, A.CreateDate,B.U_ItemCode,B.U_ItemName,B.U_Newitemcode,A.U_SUPPDCNo ,A.U_SUPPDCDt,A.U_CardCode,B.U_Length,A.U_CardName,A.Creator,B.U_BatchNo,B.U_Quantity,B.U_Mtrs from [@INT_OPDN] A INNER JOIN [@INT_PDN1] B On A.DocEntry = B.DocEntry where A.DocNum = '" & txtgrn.Text & "'  and b.u_itemcode = '" & selectedproduct & "'"
        Dim cmd1 As New SqlCommand(sql1, con)
        Dim dt1 As New DataTable()
        Dim ad1 As New SqlDataAdapter(cmd1)
        ad1.Fill(dt1)
        If dt1.Rows.Count > 0 Then
            ...
            lblgrnno.Text = txtgrn.Text
            lblgrndate.Text = grndate.ToString("dd/MM/yyyy")
            lblitemcode.Text = dt1.Rows(0)("U_itemcode").ToString()
            lblitemname.Text = dt1.Rows(0)("U_itemname").ToString()
    ...
    Public Sub loadgrid()
        Dim sql1 As String = "SELECT autonum,setno,loomno,beamno,thorneqty,thornemtrs,thronecolor FROM SUBGRNBEAM where subgrnno = '" & txtgrn.Text & "' ORDER BY AUTONUM"
        Dim cmd1 As New SqlCommand(sql1, con)
    ...
    Public Sub gettotalquantity()
        ...
        lbl1.Visible = True
        Dim sql1 As String = "select isnull(sum(thorneqty),0) as [totalqyantity] from subgrnbeam where subgrnno = '" & txtgrn.Text & "' "
        Dim cmd1 As New SqlCommand(sql1, con)
        Dim dt1 As New DataTable()
        Dim ad1 As New SqlDataAdapter(cmd1)
        ad1.Fill(dt1)
    ...
    Public Sub loaddocnum()
        ...
        If dt1.Rows.Count > 0 Then
            ...
            lbldocnum.Text = docnumber.ToString + 1
        Else
            docnumber = 0
        End If
        con.Close()
    ...
    Public Sub savewagedetails()
        ...
        cmd.Parameters.AddWithValue("NEWITEMCODE", lblnewcode.Text)
    ...
    Private Sub txtmtrs_TextChanged(sender As Object, e As EventArgs) Handles txtmtrs.TextChanged
